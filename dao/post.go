...
func GetPostPageBySlug(slug string, page, pageSize int) ([]models.Post, error) {
	rows, err := DB.Query("select * from blog_post where slug = ? limit ?,?", slug, pageSize*(page-1), pageSize)
	if err != nil {
		log.Println("GetPostPageBySlug [DB.Query] error:", err)
		return nil, err
	}
	...
	for rows.Next() {
		...
		if err != nil {
			log.Println("GetPostPageBySlug [rows.Scan] error:", err)
			return nil, err
		}
